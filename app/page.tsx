...
function ClimateSection() {
  const containerRef = useRef<HTMLDivElement>(null);
  const [temperature, setTemperature] = useState(26); // Start warm
  const [manualControl, setManualControl] = useState(false);
  const [hasStarted, setHasStarted] = useState(false);

  const isInView = useInView(containerRef, { once: true, amount: 0.4 });

  // Gradual temperature progression when in view
  useEffect(() => {
  ...
  return (
    <>
      {/* Animations for airflow / sunbeams */}
      <style jsx global>{`
        @keyframes airFlow {
          0% {
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between bg-white/80 backdrop-blur-sm rounded-2xl p-4 border border-white/20">
                ...
                <div className="flex items-center space-x-3">
                  <button
                    onClick={() =>
                      handleTempChange(Math.max(16, temperature - 1))
                    }
                  ...
                  <span
                    className={`text-2xl font-light min-w-[60px] text-center transition-all duration-500 ${
                      mode === "cool"
                        ? "text-blue-600"
                        : mode === "warm"
                        ? "text-orange-600"
                  ...
                  <button
                    onClick={() =>
                      handleTempChange(Math.min(30, temperature + 1))
                    }
                    className="w-8 h-8 rounded-full bg-blue-100 hover:bg-blue-200 flex items-center justify-center text-blue-600 font-bold transition-colors"
                  >
                    +
                    ...
                    {[...Array(4)].map((_, i) => (
                      <div
                        ...
                        style={{
                          ...
                          width: "300px",
                          height: "4px",
                          background: `linear-gradient(90deg, transparent 0%, rgba(255, 193, 7, 0.1) 30%, rgba(255, 152, 0, 0.15) 70%, transparent 100%)`,
                          animation: `sunbeamSubtle ${
                            8 + i * 0.8
